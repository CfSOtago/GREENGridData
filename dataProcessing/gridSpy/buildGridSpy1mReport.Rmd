 ...
 * Anderson, B. (`r 1900 + as.POSIXlt(Sys.Date())$year`) Processing report: NZ GREEN Grid project 1 minute electricity  power data, University of Otago: Dunedin, NZ.
...
This report is intended to produce summary data quality statistics for the original GREEN grid Grid Spy data.

The cleaned data has _no_ identifying information such as names, addresses, email addresses, telephone numbers and is therefore safe to share across all partners.
 ...
 * grid spy 1 minute data downloads processed using https://github.com/dataknut/nzGREENGridDataR/blob/master/dataProcessing/gridSpy/processGridSpy1mData.R

## History

Generally tracked via our github [repo](https://github.com/dataknut/nzGREENGridDataR/issues):

 * [history](https://github.com/dataknut/nzGREENGridDataR/commits/master)
 * [issues](https://github.com/dataknut/nzGREENGridDataR/issues)
...
We do not 'support' the code but if you have a problem check the [issues](https://github.com/dataknut/nzGREENGridDataR/commits/master) on our [repo](https://github.com/dataknut/nzGREENGridDataR) and if it doesn't already exist, open one. We might be able to fix it :-)

# Original Data: Quality checks

In this section we generate a listing of all 1 minute data files that we have received. These files are stored on `r ggrParams$otagoHCS` at:
...
ofile <- paste0(gSpyOutPath, "/checkStats/circuitLabelMetaDataCheckTable.csv")
write.csv(dt, ofile)
```
...
# get file list 
hhStatDT <- data.table::as.data.table(readr::read_csv(hhStatsByDate))
myCaption <- paste0(myCaption,
                        "\nOnly files of size > ", 
                        gSpyFileThreshold, " bytes loaded")
...
ggplot2::ggsave(paste0(gSpyOutPath, "/checkPlots/gridSpyLoadedFileNobsTilePlot.png"))
...
ggplot2::ggsave(paste0(gSpyOutPath, "/checkPlots/gridSpyLoadedFileNobsPointPlot.png"))
```
...
ggplot2::ggsave(paste0(gSpyOutPath, "/checkPlots/gridSpyLiveHouseholdsToDate.png"))
