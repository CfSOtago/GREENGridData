...
> NB: The table is only legible in the html version of this report because latex does a very bad job of wrapping table cell text. A version is saved in `r paste0(statsLoc, "circuitLabelMetaDataCheckTable.csv")` for viewing in e.g. xl.
...
dt$newx = stringr::str_wrap(dt$circuitLabels, width = 20) #https://stackoverflow.com/questions/21878974/auto-wrapping-of-labels-via-labeller-label-wrap-in-ggplot2
...
ggplot2::ggplot(dt, aes(y = newx, x = hhID, 
  ...
  labs(title = "Circuit label counts for all loaded grid spy data",
       x = "Circuit label sets (as strings)",
       caption = paste0(myCaption,
                        "\nOnly files of size > ", 
