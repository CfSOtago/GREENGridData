...
statsLoc <- paste0(ggrParams$projLoc, "/dataProcessing/gridSpy/checkStats/")
...
> NB: As before, the table is only legible in the html version of this report because latex does a very bad job of wrapping table cell text. A version is saved in `r paste0(statsLoc, "circuitLabelMetaDataCheckTable.csv")` for viewing in e.g. xl.

```{r circuitLabelMetaDataCheck}
knitr::kable(dt, 
             caption = "Circuit labels by household with additional meta-data")

ofile <- paste0(statsLoc, "circuitLabelMetaDataCheckTable.csv")
write.csv(dt, ofile)
```
