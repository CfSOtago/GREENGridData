 ...
 * Anderson, B. (`r 1900 + as.POSIXlt(Sys.Date())$year`) `r params$title`: `r params$subtitle`, `r ggrParams$pubLoc`. 
...
The original data files files are stored on `r ggrParams$otagoHCS`. The data used to generate this report was loaded from:
...
To date we have `r tidyNum(nFiles)` files with `r tidyNum(uniqueN(fListCompleteDT$hhID))` unique grid spy IDs
