...
plotDT <- powerDT[imputedTotalW <= 0, .(minW = min(imputedTotalW)), keyby = .(season, 
                                                obsTimeHMSQHour, 
                                                linkID)
             ]
 ...
 * v1.1:
   ...
   * excluding other -ve values on a per-value basis;
   * only including households known to have PV if analysis of demand during non-daylight hours is being undertaken (c.f. Figures \@ref(fig:makeMMeanPlot) and \@ref(fig:makeMinPlot)).
  
In _all_ cases we recommend that users check the data carefully before anlaysis and _document_ anY filtering they apply!
