...
Any duplicate observations were then removed using `linkID` <-> `r_dateTime` <-> `circuit` <-> `powerW`
 ...
 * data which was originally stored as NZT (`dateTime_orig` is NZT so `TZ_orig` == "date NZ") will already have had duplicate (or missing) times during the DST breaks. The data processing code will have attempted to convert the duplicates to identical UTC moments in time and any _exact_ duplicates will have been 'accidentally' removed as above. This may also cause confusion.

To add even more confusion it is _possible_ that attempts were made to 'correct' the time stamps in the DST breaks in the original data before it was downloaded by the research team. As it is almost iumpossible for us to determine what was, or should be done to correct this we:

 * have retained the original timestamp in the data in the `dateTime_orig` column;
 * have flagged the original date format in the data in the `TZ_orig` column;
...
Now set a dateTime that does not exist as it lies inside the DST break (see \@ref(tab:dstBreaks)):
...
Yes, we love timezones and DST. So:
