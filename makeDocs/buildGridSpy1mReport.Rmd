...
rmdLibs <- c("data.table", # data munching
             ...
             "kableExtra" # for fancier tables (load after knitr)
)
# load them
...
In addition two of the GridSpy units [were re-used in new households](ggOverviewReport.html#3_study_recruitment) following withdrawal of the original participants. The GridSpy IDs (rf_XX) remained unchanged despite allocation to different households. The original input data does not therefore distinguish between these households and we discuss how this is resolved in the clean safe data in Section \@ref(reallocation) below.
...
kable(caption = cap, head(aList, 10), digits = 2) %>%
  kable_styling()
```
...
kable(t, caption = "Summary of household files to load", digits = 2) %>%
  kable_styling()
```
...
```{r summaryTable}
kable(t,caption = "Summary observation stats by hhID", digits = 2) %>%
  kable_styling()
```
...
kable(dt, caption = "Circuit labels list by number of files per household", digits = 2) %>%
  kable_styling()
```
