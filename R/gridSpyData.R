...
#' Loads cleaned grid spy power data for a given circuit between two dates into a data.table
#'
#' \code{extractCleanGridSpyCircuitV2} loops over all clean household grid spy data files and loads each in turn using \code{readr::read_csv}. It filters each file to
#' extract data for particular circuits between two dates and creates some derived time/date variables before
#' using \code{rbind} to create a single data.table for each circuit label which is saved but not returned.
#'
#' Function matches circuit labels in \code{circuitPattern} to extract specific circuits and selects observations between
#'  \code{dateFrom} and \code{dateTo}. Use this to extract 1 or more circuits you want between any given dates.
#'
#'  Each element of \code{circuitPattern} is passed to the \code{data.table} operator \code{\%like\%} so wild cards & stuff may work. YMMV
#'
#'  Use of \code{readr::read_csv} enables .gz files to be autoloaded and proper parsing of dateTimes.
#'
#' @param fPath location of files to load
#' @param exFile location to save the results
#' @param circuitPattern the circuit pattern(s) to match as a properly formed R list such as \code{circuitPattern <- c("Heat Pump", "Hot Water")}
#' @param dateFrom date to start extract (inclusive)
#' @param dateTo date to end extract (inclusive)
#'
#' @import data.table
...
#'
#'
extractCleanGridSpyCircuitV2 <- function(fPath, exFile, circuitPattern, dateFrom, dateTo) {
  # check files to load
  fPattern <- "*.csv.gz"
  print(paste0("#-> Looking for data using pattern = ", fPattern, " in ", fPath, " - could take a while..."))
  #> Get the file list as a data.table ----
