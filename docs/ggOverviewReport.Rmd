...
The project research sample comprises `r nrow(hhDT)` households who were recruited in two batches, one starting in May 2014 and the second starting in November 2014 via two local lines companies. 

Recruitment was via a non-random sampling method and a number of households were intentionally selected for their 'complex' electricity consumption (and embedded generation) patterns and appliances [@stephenson_smart_2017, @jack_minimal_2018, @suomalainen_comparative_2017]. As a result the sample cannot be assumed to represent the population of customers of either company, nor of the populations in each location [@stephenson_smart_2017].
...
```{r allHhData}
kable(hhDT[, c("hhID", "linkID", "Location", "notes", "r_stopDate")][order(linkID)], caption = "Sample details")
